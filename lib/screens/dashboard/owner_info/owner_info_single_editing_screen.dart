...
class OwnerInfoSingleEditingScreen extends StatefulWidget {
  ...
  @override
  _OwnerInfoSingleEditingScreenState createState() =>
      _OwnerInfoSingleEditingScreenState();
}

class _OwnerInfoSingleEditingScreenState
    extends State<OwnerInfoSingleEditingScreen> {
                      ...
                      GestureDetector(
                        onTap: () async {
                          ...
                          if (widget.ownerNumber == 1) {
                            if (updatedPet.contacts != null &&
                                updatedPet.contacts.length > 0) {
                              updatedPet.contacts[0] = updatedOwner;
                            } else {
                              updatedPet.contacts = <Owner>[];
                              updatedPet.contacts.add(updatedOwner);
                            }
                          } else {
                            if (updatedPet.contacts != null &&
                                updatedPet.contacts.length > 1) {
                              updatedPet.contacts[1] = updatedOwner;
                            } else {
                              if (updatedPet.contacts == null) {
                                updatedPet.contacts = <Owner>[];
                              }
                              ...
                              child: Padding(
                                ...
                                child: Column(
                                  ...
                                  children: [
                                    ...
                                    TextFormField(
                                      validator: (value) =>
                                          ValidatorHelper.addressValidator(
                                              value),
  ...
  void setUpInputControllers() {
    if (widget.ownerNumber == 1 &&
        widget.currentPet.contacts != null &&
        widget.currentPet.contacts.length > 0) {
      _ownerName = new TextEditingController(
          text: widget.currentPet.contacts[0].name.value);
      _ownerEmail = new TextEditingController(
          text: widget.currentPet.contacts[0].email.value);
      _ownerAddress = new TextEditingController(
          text: widget.currentPet.contacts[0].address.value);
    } else {
      if (widget.ownerNumber == 2 &&
          widget.currentPet.contacts != null &&
          widget.currentPet.contacts.length > 1) {
        _ownerName = new TextEditingController(
            text: widget.currentPet.contacts[1].name.value);
        _ownerEmail = new TextEditingController(
            text: widget.currentPet.contacts[1].email.value);
        _ownerAddress = new TextEditingController(
            text: widget.currentPet.contacts[1].address.value);
      } else {
        _ownerName = new TextEditingController();
        _ownerEmail = new TextEditingController();
        _ownerAddress = new TextEditingController();
  ...
  void getPhoneNumbers() async {
    if (widget.ownerNumber == 1 &&
      ...
      _initialOwnerPhoneNumber = await PhoneNumber.getRegionInfoFromPhoneNumber(
          widget.currentPet.contacts[0].phoneNumber.value);
      ...
      if (widget.ownerNumber == 2 &&
        ...
        _initialOwnerPhoneNumber =
            await PhoneNumber.getRegionInfoFromPhoneNumber(
                widget.currentPet.contacts[1].phoneNumber.value);
